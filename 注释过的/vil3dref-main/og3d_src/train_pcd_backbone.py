...
class PcdClassifier(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.obj_encoder = PcdObjEncoder(config.obj_encoder)  # 初始化对象编码器
        self.obj3d_clf_pre_head = get_mlp_head(  # 初始化分类头部
            config.hidden_size, config.hidden_size,
            config.num_obj_classes, dropout=config.dropout
        )

    def forward(self, obj_pcds):
        obj_embeds = self.obj_encoder.pcd_net(obj_pcds)  # 编码点云
        obj_embeds = self.obj_encoder.dropout(obj_embeds)  # 应用dropout
        logits = self.obj3d_clf_pre_head(obj_embeds)  # 计算分类logits
        return logits
...
# 定义collate函数，用于在DataLoader中批处理数据
