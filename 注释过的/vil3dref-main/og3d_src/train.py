...
def validate(model, model_cfg, val_dataloader, niters=None, return_preds=False):
    ...
    参数:
        ...
        nitters: 可选的，指定验证的迭代次数。如果为 None，则验证整个数据集。
        return_preds: 可选的，是否返回预测结果。默认为 False。

    返回:
        avg_metrics: 包含平均指标的字典，例如损失和准确率。
        out_preds: 如果 return_preds 为 True，则返回预测结果的字典。
    """
    ...
    avg_metrics = defaultdict(AverageMeter)  # 初始化平均指标字典
    out_preds = {}  # 初始化预测结果字典
    for ib, batch in enumerate(val_dataloader):  # 遍历验证数据批次
        batch_size = len(batch['scan_ids'])  # 获取批次大小
        result, losses = model(  # 执行模型前向传播和损失计算
            batch, compute_loss=True, is_test=True,
            output_attentions=output_attentions,
            output_hidden_states=False,
        )

        loss_dict = {'loss/%s' % lk: lv.data.item() for lk, lv in losses.items()}  # 将损失转换为字典
        for lk, lv in loss_dict.items():  # 更新平均损失指标
            avg_metrics[lk].update(lv, n=batch_size)
        ...
        if return_preds:  # 如果需要返回预测结果
            for ib in range(batch_size):
                ...
                if output_attentions:  # 如果需要返回注意力权重
        ...
        if niters is not None and ib >= niters:  # 如果指定了验证迭代次数
            break
    LOGGER.info(', '.join(['%s: %.4f' % (lk, lv.avg) for lk, lv in avg_metrics.items()]))  # 记录平均指标
